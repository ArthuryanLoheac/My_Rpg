    ...
    guis_t *gui, game_t *game, page_shop_t *options)
{
    ...
    if (button->state == RELEASED) {
        ...
        sfView_setCenter(((map_t *)(game->guis->map->ui_content))->view,
            sfSprite_getPosition(((map_t *)(game->guis->map->ui_content))
...
static void check_compute_button(page_shop_t *options, enum class_e class)
{
    ...
    if (options->state != CYBERWARE_SHOP && class == TECH)
        update_state_button_cyberware(options->Cyberware, options);
}

void compute_shop_in_game(ui_obj_t *obj, game_t *game)
{
    ...
    check_compute_button(options, game->player->class);
